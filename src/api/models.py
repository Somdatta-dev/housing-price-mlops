...
class PredictionResponse(BaseModel):
    ...
    confidence_score: Optional[float] = Field(
        None, description="Model confidence score (0-1) if available", ge=0.0, le=1.0
    )
    input_features: Dict[str, float] = Field(
        ..., description="Input features used for prediction"
    )
    processing_time_ms: Optional[float] = Field(
        None, description="Time taken to process prediction in milliseconds"
    )
...
class BatchPredictionRequest(BaseModel):
    ...
    batch_name: Optional[str] = Field(
        None, description="Optional name for the batch", max_length=100
    )
...
class BatchPredictionResponse(BaseModel):
    ...
    batch_name: Optional[str] = Field(None, description="Name of the batch if provided")
...
class ModelInfo(BaseModel):
    ...
    feature_importances: Optional[Dict[str, float]] = Field(
        None, description="Feature importances (for tree-based models)"
    )
