...
class MetricsCollector:
    ...
    def record_api_request(self, endpoint: str, method: str, status_code: int,
                          response_time_ms: float, request_size: int = None,
                          response_size: int = None):
        """Record API request metrics"""
        timestamp = datetime.utcnow().isoformat() + "Z"
        
        # Record individual metrics
        self.record_counter("api.requests.total", tags={
            "endpoint": endpoint,
            "method": method,
            "status": str(status_code)
        })
        
        self.record_histogram("api.response_time_ms", response_time_ms, tags={
            "endpoint": endpoint,
            "method": method
        })
    ...
    def record_model_prediction(self, model_name: str, model_version: str,
                               prediction_time_ms: float, prediction_value: float,
        ...
        # Record individual metrics
        self.record_counter("model.predictions.total", tags={
            "model_name": model_name,
            "model_version": model_version
        })
        
        self.record_histogram("model.prediction_time_ms", prediction_time_ms, tags={
            "model_name": model_name,
            "model_version": model_version
        })
        
        self.record_histogram("model.prediction_value", prediction_value, tags={
            "model_name": model_name,
            "model_version": model_version
        })
        
        if confidence_score is not None:
            self.record_histogram("model.confidence_score", confidence_score, tags={
                "model_name": model_name,
                "model_version": model_version
            })
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {
                    ...
                    "api": {
                        ...
                        "error_rate": round((api_stats[2] or 0) / max(api_stats[0] or 1, 1) * 100, 2)
                    },
                    "model": {
...
class AlertManager:
    ...
    def check_alerts(self):
        ...
        for rule in self.alert_rules:
            ...
            cutoff_time = (current_time - timedelta(minutes=rule["duration_minutes"])).isoformat() + "Z"
            ...
            try:
                with sqlite3.connect(self.metrics_collector.db_path) as conn:
                    ...
                    if result and result[0] is not None:
                        ...
                        # Check threshold
                        triggered = False
                        if rule["comparison"] == "greater" and avg_value > rule["threshold"]:
                            triggered = True
                        elif rule["comparison"] == "less" and avg_value < rule["threshold"]:
                            triggered = True
                        elif rule["comparison"] == "equal" and abs(avg_value - rule["threshold"]) < 0.001:
                            triggered = True
    ...
    def _resolve_alert(self, alert_name: str):
        ...
        if alert_name in self.active_alerts:
            ...
            self.active_alerts[alert_name]["resolved_at"] = datetime.utcnow().isoformat() + "Z"
