...
def train_linear_regression(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    with mlflow.start_run(run_name="Linear_Regression"):
        ...
        logger.info(f"Linear Regression - Val RMSE: {metrics['val_rmse']:.2f}, Val R²: {metrics['val_r2']:.3f}")
...
def train_ridge_regression(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    with mlflow.start_run(run_name="Ridge_Regression"):
        ...
        logger.info(f"Ridge Regression - Val RMSE: {metrics['val_rmse']:.2f}, Val R²: {metrics['val_r2']:.3f}")
...
def train_random_forest(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    with mlflow.start_run(run_name="Random_Forest"):
        ...
        # Grid search
        grid_search = GridSearchCV(
            rf, param_grid, cv=3,  # Reduced CV for faster training
            scoring='neg_mean_squared_error', n_jobs=-1
        )
        ...
        plot_path = create_feature_importance_plot(best_model, feature_names, "Random Forest")
        if plot_path:
        ...
        logger.info(f"Random Forest - Val RMSE: {metrics['val_rmse']:.2f}, Val R²: {metrics['val_r2']:.3f}")
...
def train_decision_tree(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    with mlflow.start_run(run_name="Decision_Tree"):
        ...
        # Create and log feature importance plot
        feature_names = X_train.columns.tolist()
        plot_path = create_feature_importance_plot(best_model, feature_names, "Decision Tree")
        if plot_path:
            mlflow.log_artifact(plot_path)
            os.remove(plot_path)  # Clean up
        ...
        logger.info(f"Decision Tree - Val RMSE: {metrics['val_rmse']:.2f}, Val R²: {metrics['val_r2']:.3f}")
...
def main():
    ...
    # Linear Regression
    try:
        model, metrics = train_linear_regression(X_train, y_train, X_val, y_val, X_test, y_test)
    ...
    # Ridge Regression
    try:
        model, metrics = train_ridge_regression(X_train, y_train, X_val, y_val, X_test, y_test)
    ...
    # Decision Tree
    try:
        model, metrics = train_decision_tree(X_train, y_train, X_val, y_val, X_test, y_test)
